...
st.subheader("🎴 Tirage des cartes")
...
if st.button("Tirer les cartes"):
    ...
    if type1 == type2:
        ...
        couleur = "green" if effet["type"] == "Bénéfique" else "red"

        st.markdown(f"### Effet {effet['type']}")
        st.markdown(effet["effet_principal"])
    ...
    # Cas mixte
        ...
        jet_chance = st.selectbox("Résultat du jet de Chance", ["Réussi", "Raté"])

        if jet_chance:
            if jet_chance == "Réussi":
    ...
    st.success(f"Durée de la prédiction : {table_duree[jet_duree]}")
